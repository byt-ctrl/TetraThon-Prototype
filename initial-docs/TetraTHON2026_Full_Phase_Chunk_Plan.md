...
Precision Crop Advisory System & Post-Harvest Loss Reduction Planner **Simple Phase-Wise Build Plan — 4 People, 4 Chunks, One Person at a Time** 

Every phase is split into exactly 4 chunks. Each chunk lists exactly what to build. _Prototype (Week 1) → Hackathon MVP (Week 2) → Pilot → Scale_ Version 1.0 | July 17, 2026 
...
## 2. Phase 0 — Prototype (Week 1, Days 1–7) 
...
**Owner: P1** |   Timing: Day 1 (1 day) 

_You are building the skeleton that everyone else builds on top of. Nothing fancy — just a working, empty app that is live on the internet, plus the base data everyone else needs._ 
...
7. Write down the two user flows in plain text or a simple diagram: (a) the Advisory flow, (b) the PostHarvest flow — this becomes the architecture diagram later. 
...
**Owner: P2** |   Timing: Day 2–3 (2 days) 

_You are building the first real feature: a farmer fills a form and gets 3 ranked advisories. Everything runs on fake weather data for now — that becomes real in Week 2._ 
...
**Owner: P3** |   Timing: Day 4–5 (2 days) 

_You are building the second feature: a farmer enters their harvest details and gets a Sell / Store / Transport recommendation. This must not break anything Chunk 2 built._ 
...
**Owner: P4** |   Timing: Day 6–7 (2 days) 

_You are connecting both modules into one clean demo and getting everything ready to submit. This chunk is split into two halves: build (Day 6) and submission prep (Day 7)._ 

#### **What to build, step by step:** 

1. Day 6 — Build a single dashboard screen where you pick one farmer/crop scenario and see both Module A's advisories and Module B's recommendation together, side by side. 

2. Day 6 — Add two simple line charts to the dashboard: one showing the spoilage curve over time, one showing the price trend — use Recharts or Chart.js. 

3. Day 6 — Do a UI/UX pass: consistent colours and spacing, use icons instead of dense text where possible, and check the layout works on a phone screen. 

4. Day 6 — Write a clear README: what the project does, how to run it, and 2–3 screenshots. Draw a simple architecture diagram (boxes: Frontend → Backend → Rule Engine → Data). Record a short demo GIF or video. 

5. Day 7 (morning) — Build the submission PPT: Title & team, Problem Understanding, Approach (architecture + tech stack + data flow), Demo screenshots, Market Fit & Real-World Relevance, Roadmap (Phase 0 → 1 → 2), Closing slide with links. 

6. Day 7 (morning) — Rehearse the demo out loud at least twice. Re-test all 4×5 crop/location combinations one final time. 

7. Day 7 (morning) — Ask someone outside the team to try the demo cold. Fix anything that confuses them. 

8. Day 7 (afternoon) — Submit the PPT and the GitHub link. Keep this time as buffer — don't plan new work into it. 
...
## 3. Phase 1 — Hackathon MVP (Week 2, Days 8–14) 

This phase starts only once your team is selected and receives the full problem statement at the July 31 inauguration. Goal of this phase: replace the fake/mocked parts with real ones, and add the stretch features that were skipped in Week 1. The same rotation continues — Person 1 leads again. 

_If the organisers announce a much shorter on-site window (for example 24–48 hours) instead of a full week, shrink each chunk below to roughly a quarter of the days shown, in the same order, and drop the lowest-priority steps first (leaf classifier and SMS alert are the first to trim)._ 

### **Chunk 5 — Real Data — Weather & Market Prices** 

**Owner: P1** |   Timing: Day 8–9 (2 days) 

_You are replacing the fake weather and price data from Week 1 with real, live data — but keeping the fake data as a safety net in case the internet or an API fails during the demo._ 

#### **What to build, step by step:** 

1. Read the full hackathon problem statement carefully and note anything that changes what you need to build compared to the Week 1 prototype. 
...
4. Add a fallback: if the live weather call fails or times out, automatically use Week 1's mocked data instead, silently — the user should never see an error screen. 
...
7. Deploy your changes on top of Chunk 4's live app. Confirm nothing from Week 1 broke. 
...
**Owner: P2** |   Timing: Day 10–11 (2 days) 

_You are making the leaf-photo upload button (built in Chunk 2 but unused) actually work — a real, if simple, AI model that looks at a leaf photo and predicts a disease or pest._ 
...
**Owner: P3** |   Timing: Day 12 (1 day) 

_You are making the price-alert feature actually send a message, using a free sandbox — no real telecom cost or setup needed._ 
...
4. Build a background job that checks current prices against stored thresholds (for the demo, this can be a button you click, or a job that runs every couple of minutes). 
...
**Owner: P4** |   Timing: Day 13–14 (2 days) 

_You are making the whole app solid and making sure the team can demo it confidently in front of judges, twice in a row, without anything breaking._ 

#### **What to build, step by step:** 

1. Day 13 — Write automated tests (pytest) covering the main logic: the advisory ranking engine, the spoilage model, and the decision engine. Cover the normal cases and a few edge cases. 

2. Day 13 — Set up GitHub Actions so these tests run automatically on every push, and block merging if a test fails. 

3. Day 13 — Set up Sentry (or a similar free tool) on both frontend and backend so errors get logged automatically during the demo. 

4. Day 13 — Do a final UI polish pass: fix any rough edges, re-check the layout on a phone screen. 

5. Day 14 — Write and rehearse a demo script: the exact order you'll click through, and the key talking points for problem, approach, and market fit. 

6. Day 14 — Prepare for judge Q&A: write down 5–10 questions you think judges might ask, and agree on answers as a team. 

7. Day 14 — Run the full demo twice back-to-back to confirm nothing breaks the second time (stale data, leftover test alerts, etc.). 
...
## 4. Phase 2 — Pilot (Month 1–3 After the Hackathon) 

This phase only happens if you're moving forward with a real FPO (Farmer Producer Organisation) or KVK (Krishi Vigyan Kendra) partner after the hackathon. Goal: turn the hackathon MVP into something a real partner organisation can actually use with real farmers. Each chunk here takes about 2–3 weeks instead of days, since it's real infrastructure work. 

### **Chunk 9 — Move to Real Cloud Infrastructure** 

**Owner: P1** |   Timing: ≈2–3 weeks 

_You are moving the app off free hackathon-tier hosting onto proper, managed cloud infrastructure that can handle real users._ 
...
**Owner: P2** |   Timing: ≈2–3 weeks 

_You are adding the ability for a field agent (who works with many farmers) to log in and manage records for multiple farmers, in their own language._ 
...
**Owner: P3** |   Timing: ≈2–3 weeks 

_You are replacing the sandbox alert system with real messaging, and starting to collect real feedback from farmers on whether the advisories are actually useful._ 
...
**Owner: P4** |   Timing: ≈2–3 weeks 

_You are making the app reliable enough for real, everyday use in the field, and handing it over to the pilot partner organisation._ 
...
## 5. Phase 3 — Scale (Month 3–12 After the Hackathon) 

This is the long-term phase: growing from one pilot partner to many, and from a rule-based system to one that learns from real data. Each chunk here takes about 4–6 weeks. 

### **Chunk 13 — Split Into Separate Services** 

**Owner: P1** |   Timing: ≈4–6 weeks 

_You are breaking the single app into smaller, independent services so the system can grow without one part slowing down the others._ 
...
**Owner: P2** |   Timing: ≈4–6 weeks 

_You are building a proper mobile app so farmers and field agents don't need to rely on a mobile browser._ 
...
**Owner: P3** |   Timing: ≈4–6 weeks 

_You are starting to replace the rule-based logic with models trained on real data collected from the pilot, while keeping a safe fallback._ 
...
**Owner: P4** |   Timing: ≈4–6 weeks 

_You are making the system ready to support multiple FPOs/KVKs at the same time, not just the original pilot partner._ 
...
## 6. Quick Reference — Who Builds What, When 

|**Phase**|**P1**|**P2**|**P3**|**P4**|
|---|---|---|---|---|
|Phase 0 — Prototype<br>(Week 1, Day 1–7)|Foundation +<br>Rules (Day 1)|Crop Advisory<br>Module (Day 2–3)|Post-Harvest<br>Module (Day 4–5)|Integration +<br>Submission (Day<br>6–7)|
|Phase 1 — Hackathon<br>MVP (Week 2, Day 8–14)|Live Weather +<br>Prices (Day 8–9)|Leaf Classifier (Day<br>10–11)|Real Price Alert<br>(Day 12)|Testing + Demo<br>Ready (Day 13–14)|
|Phase 2 — Pilot (Month<br>1–3)|Cloud<br>Infrastructure|Field-Agent +<br>Language|Feedback + Real<br>Messaging|Hardening +<br>Handover|
|Phase 3 — Scale (Month<br>3–12)|Split Into Services|Mobile App|Smarter Models|Multi-Partner<br>Support|



_Notice the pattern: it's always P1 → P2 → P3 → P4, in every phase. Nobody is stuck doing only frontend or only backend — everyone gets a turn building a complete feature from scratch._ 
...
- If you get stuck, don't sit on it — timebox the problem (1–2 hours in Week 1–2, a bit more later) and then ask the next person in line to help unblock you. 
